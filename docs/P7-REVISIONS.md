...
- [x] **リモートの戻るで HOME の前に接続一覧へ** (`add1461`→`be2ac80`→`e5ab221`, 0.2.6→0.2.7→0.2.8): FILES ルートで戻ったとき、いまリモートを開いていれば HOME ではなく接続一覧へ戻す。
  - 0.2.6: 「接続タブから開いたか」を `MainScreen` の `remember` フラグで持たせたが、**アクティビティ再生成で初期値に戻り**依然 HOME へ。
  - 0.2.7: フラグを `rememberSaveable` 化したが**それでも HOME へ** (実機で再現)。フラグ方式 (開いた時点で記録) が経路により取りこぼすと判断。
  - 0.2.8: **フラグを廃止し、戻る瞬間に `browserViewModel.state` の breadcrumbs の根が `FileUri.Remote` かを直接判定**する方式へ。リモート表示中ならどの経路で開いても接続一覧へ戻す (ローカル/SAF は従来どおり HOME)。← 現行。

### 引き継ぎメモ (第6回追記)
- **push**: §M までの未 push 分を含め、本 §N の 9 コミット (`d3cf078` `5ecfaad` `50e7544` `39a204f` `353b9c0` `add1461` `be2ac80` `e5ab221` + 本 docs) を `origin/phase/P7-polish` へ push。
...
- **実機状態 (Motorola moto g66j 5G, ja-JP, Android 14)**: release `com.zerotoship.foldex` を 0.2.8 で再インストール済み。
- **未解決・要実機確認**: ① AlarmManager 定期同期の発火 (Doze 下) は実機未確認。一部 OEM の省電力で殺される場合あり (電池最適化除外の案内は未実装)。② 永久ログは既存 `.log.txt` には影響しないため作り直しが必要。③ リモートの戻り先 (0.2.8、browser-state 方式) の実機再確認 — 0.2.6/0.2.7 のフラグ方式では直らなかった経緯あり。なお MainScreen の親 BackHandler が発火している前提 (現状 HOME に行く=発火はしている) で 0.2.8 は成立する。
