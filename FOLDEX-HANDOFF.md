...
**中身検索 (grep)** (v1.0.3〜): 検索バーの「中身」トグルで、ファイル名ではなくファイルの中身を検索する。
- 対象: テキスト系 (txt/json/csv/xml/log/md/ソース等) + Office 新形式/OpenDocument (docx/xlsx/pptx/odt/ods/odp)。
  Office は zip 内の本文 XML からタグを外して抽出する (追加ライブラリなし)。
- 非対応: PDF・旧バイナリ形式 (doc/xls/ppt)・rtf (いずれも抽出ライブラリが必要なため将来検討)。
- 対象ストレージ: 端末内のみ (ローカル + SAF)。リモートは通信が重いので対象外。
- 「サブフォルダ」トグルは中身検索にも効く。1 ファイルずつ読むため上限は 1 ファイル 16MB・最大 1000 件・キャンセル可。
- 結果はヒットしたファイル + マッチ箇所の抜粋 (スニペット) を表示。
- 既知の制限: Word が 1 単語を複数ランに分割している場合、その語はヒットしないことがある (タグ除去方式のため)。
...
| 検索 | 部分一致 + glob。現在フォルダのみ / サブフォルダ含む再帰 (P7 §Q 実装)。中身検索 (grep) は「中身」トグル、端末内のみ・テキスト+Office 対応 (v1.0.3) |
...
| **P7** | UI洗練、エラーハンドリング、テスト配布 + P8 から前倒し (双方向同期 / リモート動画ストリーミング / HOME 画面 / Sora エディタ / ZIP) | ✅ 完了 (`v0.9.0-P7`、0.2.37) |
| **P8** | F-Droid/Play配布 (SPDX ヘッダ / プライバシーポリシー / F-Droid metadata / GitHub Actions リリースワークフロー / Reproducible Build ドキュメント)、`v1.0.0` | ✅ 完了 (`v1.0.0`、1.0.0)。エクスポート/インポートと同期途中再開は将来検討 |
...
最終更新: 2026-06-28 (バグ修正リリース: 空フォルダでプルダウン更新が発火しない不具合を修正 — EmptyContent を LazyColumn 化し PullToRefreshBox のネストスクロールに参加させた。バージョン 1.0.1 / versionCode 42)
