...
最終更新: 2026-06-14 (P7 第9回 §Q を機能表に反映 (検索の再帰 / 使用量分析 / 同期実行直列化 / 削除復元プレビュー / 同期実行ログ拡張)。P7 第10回 §R / §S は内蔵エディタが空ファイルで `RESET → FLUSHED` で開けない不具合の真因確定と修正 (`CharsetDecoder` 状態機械) のみで機能変更なし。詳細は `docs/P7-REVISIONS.md` §Q / §R / §S を参照。バージョン 0.2.37 / versionCode 39)
