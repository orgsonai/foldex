...
- **再生は完全に外部に任せる** (VLC, MX Player, mpv 等の専用アプリの方が高品質)
- サムネは内蔵:
...
- **動的カラーをデフォルトオン** (Android 12+ のシステム壁紙色に追従)
...
- **デフォルトオン**
- オフ時は Foldex 独自カラーにフォールバック

#### Foldex 独自カラー
- **Forest Green** (`#2E7D32`) を seed カラーとして採用
- `ColorScheme.fromSeed(Color(0xFF2E7D32))` で調和の取れたパレットを自動生成
- Zero to Ship のアクセントカラー (推測) と整合
...
- **ゴミ箱機能**: P7以降の実装、初期は確認ダイアログ + アンドゥで運用
