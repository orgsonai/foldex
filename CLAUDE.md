...
6. **コミット前には必ずバージョンを上げてからコミットする**
   実コードを変更するコミットの前に `app/build.gradle.kts` の `versionName` (必要なら `versionCode`) を上げる。
   バージョンを上げる変更とコミット対象を同じコミットに含める。ドキュメントのみの軽微な変更は対象外で可。
