...
6. **バージョンを上げる → ビルド/テスト → コミット の順を必ず守る**
   実コードを変更するときは、**先に** `app/build.gradle.kts` の `versionName` (必要なら `versionCode`) を上げ、**その上げた状態でビルド/テストを通してからコミットする**。
   理由: ビルド検証の後にバージョンを上げると「検証した版」と「コミットした版」がズレるため。バージョンを上げてから検証すれば、緑を確認した版がそのままコミットされる。
   バージョンを上げる変更とコミット対象は同じコミットに含める。ドキュメントのみの軽微な変更は対象外で可。
...
- **手順は「バージョンを上げる → ビルド/テスト → コミット」** (§0-6)。先にバージョンを上げ、その版でビルド/テストを通してからコミットする。検証した版とコミットした版を一致させるため。
